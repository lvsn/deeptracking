...
function RGBDTracker:build_convo(input_channels, c1_filters, c2_filters, final_size)
    ...
    convo:add(nn.SpatialConvolution(c1_filters * 2, c2_filters, c2_filter_size, c2_filter_size, 2, 2))
    convo:add(nn.SpatialBatchNormalization(c2_filters))
    convo:add(nn.ELU())
    --convo:add(nn.SpatialDropout(0.2))

    convo:add(nn.SpatialConvolution(c2_filters, c2_filters, c2_filter_size, c2_filter_size, 2, 2))
    convo:add(nn.SpatialBatchNormalization(c2_filters))
    convo:add(nn.ELU())
    --convo:add(nn.SpatialDropout(0.2))
